"""
Populate player season averages tables for each team.
Creates one table per team: {team_schema}.player_season_averages

Columns: player_name, games_played, minutes_avg, points_avg, rebounds_avg, 
         assists_avg, steals_avg, blocks_avg, turnovers_avg, fg_pct, 
         three_pt_pct, ft_pct, usage_avg, etc.
"""
...
def create_season_averages_table(conn, team_schema):
    ...
    conn.commit()
    cursor.close()
    print(f"✅ Created table: {team_schema}.player_season_averages")
...
def populate_team_averages(conn, team_id, team_schema, team_name):
    ...
    # Create table
    create_season_averages_table(conn, team_schema)
    ...
    for idx, player in roster_df.iterrows():
        ...
        # Extract relevant stats (matching API column names)
        ...
        print(f"✅ {min_avg:.1f} MPG, {pts_avg:.1f} PPG")
        players_added += 1
    ...
    print(f"\n✅ {team_name}: Added {players_added}/{len(roster_df)} players")


def main():
    print("🏀 Populating Player Season Averages Tables")
    ...
    for team in sorted(all_teams, key=lambda x: x['full_name']):
        ...
        try:
            populate_team_averages(conn, team_id, team_schema, team_name)
            teams_completed += 1
        except Exception as e:
    ...
    print(f"\n{'='*80}")
    print(f"✅ COMPLETE: Processed {teams_completed}/{len(all_teams)} teams")
